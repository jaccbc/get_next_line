...
/*   Updated: 2024/10/15 15:51:11 by joandre-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (!lst[fd])
		return (NULL);
	line = clean_buffer(get_line_size(lst[fd]) + 1);
	if (!line)
		return (NULL);
	copy_line(lst[fd], line);
	lst[fd] = clear_list(lst[fd]);
	return (line);
}
